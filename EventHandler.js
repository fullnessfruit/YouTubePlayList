...
function OnWebViewTranslationDidFrameFinishLoad() {
	...
	if (webViewTranslation.getURL().startsWith("https://www.youtube.com/@") || webViewTranslation.getURL().startsWith("https://www.youtube.com/c/") || webViewTranslation.getURL().startsWith("https://www.youtube.com/user/"))
	{
		webViewTranslation.executeJavaScript("ytInitialData?.metadata?.channelMetadataRenderer?.externalId").then(function(value) {
			webViewTranslation.loadURL("https://www.youtube.com/playlist?list=UU" + value.slice(2));
		});
	}
}
